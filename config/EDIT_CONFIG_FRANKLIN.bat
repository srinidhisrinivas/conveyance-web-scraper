...
function load(){
   ...
   document.getElementById("year").value = JSON.StringOf("VALUE_YEAR");
...
function restore(){
   ...
   var defYear = JSON.StringOf("DEFAULT_VALUE_YEAR");
   var codesString = new ActiveXObject("Chilkat_9_5_0.StringBuilder");
   ...
   JSON.UpdateString("VALUE_YEAR", defYear);
...
function update(){
   ...
   if(!FS.FolderExists(dir)){
      ...
      document.getElementById("year-description").innerText = "New Year Saved!";


      JSON.UpdateString("TARGET_DIR", dir);
      JSON.UpdateString("VALUE_YEAR", year);
...
<BODY onload="load()">
   ...
   <div class="input-label" id="year-label" style="font-weight: 400;"> Value Year: </div> <br>
   <input type="text" id="year" style="width: 450px;"> <br>
   <div class="input-label" id="year-description" style="font-size: 8pt;"> The year for which auditor has appraised property value. </div> <br>
